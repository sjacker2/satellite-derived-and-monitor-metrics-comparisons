...
def process_nc_file(sat_file, unc_file):
    # Load satellite data; change lat and lon to the boundaries of area of interest
    sat_data = xr.open_dataset(sat_file).sel(lat=slice(10, 72), lon=slice(-180., -50)).to_dataframe()
    
    # Load uncertainty data; change lat and lon to the boundaries of area of interest
...
# Combine all filtered dataframes, groupby location, find mean
print('combined')
mean_df = combined_df.groupby(['lat', 'lon'])['GWRPM25'].mean().reset_index()

print('coverting to shapefile')
#convert to a point object shapefile
...
df_gpd.to_file('/data/acker/ALA/GL/GLv5_GWRPM25_mean2021-2023.shp') #change this to your own data directory
print('all done')
#print(mean_df)
